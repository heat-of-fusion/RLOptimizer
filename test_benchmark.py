...
from DLModel import FCNet, CNNet
...
retrain_loader = pkl.load(open(f'retrain_loader.pkl', 'rb'))
revalid_loader = pkl.load(open(f'revalid_loader.pkl', 'rb'))

print(retrain_loader[0][1].argmax(1))

retrain_loader = list(retrain_loader)
revalid_loader = list(revalid_loader)

dl_env = IntegratedEnvironment(batch_shape[1], HIDDEN_SIZE, N_CLASSES, CNNet, OptimAgentDemoV2, DL_LR)
dl_env.set_loaders(retrain_loader, revalid_loader)
dl_env.set_model()

adam_env = IntegratedEnvironment(batch_shape[1], HIDDEN_SIZE, N_CLASSES, CNNet, torch.optim.Adam, DL_LR)
adam_env.set_loaders(retrain_loader, revalid_loader)
adam_env.set_model()

sgd_env = IntegratedEnvironment(batch_shape[1], HIDDEN_SIZE, N_CLASSES, CNNet, torch.optim.SGD, DL_LR)
sgd_env.set_loaders(retrain_loader, revalid_loader)
sgd_env.set_model()

adagrad_env = IntegratedEnvironment(batch_shape[1], HIDDEN_SIZE, N_CLASSES, CNNet, torch.optim.Adagrad, DL_LR)
adagrad_env.set_loaders(retrain_loader, revalid_loader)
...
rmsprop_env.set_loaders(retrain_loader, revalid_loader)
...
# beta_1_action_map = np.array([1e-4, 0.25, 0.5, 0.75, 0.99]).astype(np.float32)
# beta_2_action_map = np.array([1e-4, 0.25, 0.5, 0.75, 0.99]).astype(np.float32)
# gamma_3_action_map = np.array([1e-4, 0.25, 0.5, 0.75, 0.99]).astype(np.float32) * 2

episode_rewards = list()

for episode in [0, 499, 999]:
    ...
    if benchmark:
        ...
        dl_env.load_state_dict(torch.load('./uniform_weights.pth'))
    ...
    print(rl_actor.load_state_dict(torch.load(MODEL_PATH + f'actor/episode_{episode}_actor.pth')))
    print(rl_critic.load_state_dict(torch.load(MODEL_PATH + f'critic/episode_{episode}_critic.pth')))
    ...
    for epoch in tqdm(range(MAX_EPOCHS), f'{"||BENCHMARK||" if benchmark else ""} Episode {str(episode).zfill(3)} Training...'):
        ...
        ################################################################
        # Train Loader, Valid Loader Resampling
        ################################################################
        # uni_train_loader = DataLoader(train_dataset, batch_size = 32, shuffle = True) # 수정 필요
        # uni_valid_loader = DataLoader(train_dataset, batch_size = 32, shuffle = True) # 수정 필요
        # uni_train_loader, uni_valid_loader = random_sampler.get_dataloader(TRAIN_LEN, VALID_LEN)
        # uni_train_loader = pkl.load(open(f'retrain_loader.pkl', 'rb'))
        # uni_valid_loader = pkl.load(open(f'revalid_loader.pkl', 'rb'))
    ...
    plt.savefig(f'./benchmark/benchmark_epi_{episode}.png')
    plt.close()

    plt.figure(figsize = (10, 10))
    plt.subplot(3, 1, 1)
    plt.title(f'Beta 1')
    plt.plot(beta_1_action_map[beta_1_list])
    plt.subplot(3, 1, 2)
    plt.title(f'Beta 2')
    plt.plot(beta_2_action_map[beta_2_list])
    plt.subplot(3, 1, 3)
    plt.title(f'Gamma 3')
    plt.plot(gamma_3_action_map[gamma_3_list])
    plt.savefig(f'./benchmark/parameters_epi_{episode}.png')
    plt.close()

    # print(torch.tensor(uni_train_loader).shape)
    # print(torch.tensor(uni_valid_loader).shape)